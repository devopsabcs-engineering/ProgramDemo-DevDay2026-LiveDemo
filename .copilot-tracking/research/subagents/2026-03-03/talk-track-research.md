# Talk Track Specification Research

**Research Topic**: Talk Track Specification for CIVIC Demo  
**Status**: Complete  
**Date**: 2026-03-03  
**Output File**: `TALK-TRACK.md` (repository root)

---

## Research Topics & Questions

1. Complete talk track structure with all 8 acts plus opening/closing
2. Two-presenter format with scripted dialogue examples
3. Checkpoint/tag strategy (v0.1.0 → v1.0.0)
4. Risk mitigation table format
5. Cliffhanger dramatic arc documentation
6. Key numbers for closing summary
7. Timing breakdowns with minute markers AND actual times
8. Recovery strategy for each phase

---

## Complete Talk Track Structure
...
| 💻 | EMMANUEL | On keyboard — drives all live coding |
```
...
**Demo actions:**
- **[0:00] 10:30 AM** — Open VS Code, show empty repository with only `README.md`
- **[2:00] 10:32 AM** — Open Azure Portal, navigate to `rg-dev-125`, show pre-deployed (but empty) resources
- **[4:00] 10:34 AM** — Open Azure DevOps, show empty backlog boards
- **[6:00] 10:36 AM** — Return to VS Code, read through README.md highlighting business problem
...
**Demo actions:**
- **[8:00] 10:38 AM** — Create `.github/copilot-instructions.md` via Copilot
...
| `docs/design-document.md` | API and component specifications |
...
| Tag | Checkpoint | Minute | Time | Recovery Notes |
|-----|------------|--------|------|----------------|
| `v0.1.0` | Scaffolding complete | 20 | 10:50 AM | All instruction files, MCP config, ADO work items |
| `v0.2.0` | Database schema complete | 32 | 11:02 AM | All 4 Flyway migrations |
| `v0.3.0` | Backend API complete | 52 | 11:22 AM | All 5 endpoints working |
| `v0.4.0` | Citizen Portal complete | 70 | 11:40 AM | Full submission flow working |
| `v0.5.0` | Ministry Portal complete | 87 | 1:17 PM | Full approval flow working |
| `v0.6.0` | Test suite complete | 100 | 1:30 PM | All tests passing |
| `v0.7.0` | CI/CD configuration complete | 107 | 1:37 PM | Pipeline ready |
| `v0.8.0` | Budget migration done | 115 | 1:45 PM | Intermediate full-stack checkpoint |
| `v1.0.0` | Full demo complete | 120 | 1:50 PM | Everything working |
...
Each act should include a risk table in this format:

| Risk | Likelihood | Impact | Mitigation | Recovery |
|------|------------|--------|------------|----------|
| MCP fails to connect | Medium | High | Test MCP 30 min before | Manual ADO work items |
| Copilot rate limit | Low | Medium | Pause 30 seconds | Pre-written snippets |
| Database migration fails | Medium | High | H2 console to debug | Restore from v0.1.0 |
| Spring Boot won't start | Low | High | Pre-built JAR backup | Run pre-built version |
| npm install timeout | Medium | Medium | Pre-cached node_modules | Skip to v0.4.0 |
| Test fails unexpectedly | Medium | Low | Skip failing test, note it | Continue demo |
| Audience question derails | High | Low | "Great question — parking lot" | Redirect to Q&A |

### Act 1: The Architect — Risk Table

| Risk | Likelihood | Impact | Mitigation | Recovery |
|------|------------|--------|------------|----------|
| MCP server won't start | Medium | High | Pre-test MCP, have manual ADO ready | Create work items manually via ADO |
| Copilot generates incorrect instructions | Medium | Medium | Review generated content live | Edit manually, explain the correction |
| VS Code extension crash | Low | High | Have second VS Code instance ready | Restart, checkout v0.0.0 |

### Act 2: The DBA — Risk Table
...
At the end of the demo, display this summary:
...
### Presenter Dialogue Format

```markdown
> 🎙️ **HAMMAD**: [MC dialogue here]
>
> 💻 **EMMANUEL**: [Technical response or narration]
```
...
```markdown
**Key beat (EMMANUEL)**: Description of pivotal moment
```

### Audience Engagement Format

```markdown
**Audience engagement point**: "Question for audience?"
```
...
1. **Ontario Design System integration** — CSS class names, component patterns
2. **i18next configuration** — namespace patterns, fallback languages
3. **ADO MCP tool syntax** — exact commands for Epic/Feature/Story creation
4. **Spring Boot H2 SQL Server mode** — connection string and dialect
5. **axe-core integration with Vitest** — testing setup patterns

---

## Next Research (Not Completed)

- [ ] Exact Ontario DS CSS class names for components
- [ ] i18next JSON structure for EN/FR translations
- [ ] ADO MCP Server exact API commands
- [ ] Pre-recorded video production requirements
- [ ] Audience Q&A anticipated questions
- [ ] Backup slides for major technical failures

---

## Clarifying Questions

1. Should the TALK-TRACK.md include actual code snippets, or just descriptions of what gets generated?
2. Is there a standard Ontario Government template for talk tracks that should be followed?
3. Should Q&A anticipated questions be included in the talk track document?
4. Are there specific accessibility narration requirements for the live demo?
5. Should the talk track include presenter movement/stage blocking notes?

---

## References & Evidence

- **README.md** — Source of business requirements and tech stack
- **demo-scaffolding.prompt.md** — Complete specification for demo scaffolding
...
- **Azure DevOps MCP** — MCP server for ADO integration
